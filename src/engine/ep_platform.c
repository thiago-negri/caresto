...
int ep_shared_load(struct ep_shared_game *out_shared_game, const char *path,
    ...
    // Save SO timestamp, we will use this to check whether we need to
    // reload it
    long long timestamp = ef_timestamp(path);

    // Copy SO to a new location to avoid locking the SO we build
    // +2 ("-b") +1 (NULL terminator)
    size_t new_path_len = strlen(path) + 2 + 1;
    char *new_path = (char *)ea_arena_alloc(arena, sizeof(char) * new_path_len);
    if (new_path == NULL) {
        el_critical_fmt("LIN: OOM: Could not allocate to write file %s.\n",
                        path);
        rc = -1;
        goto _err;
    }
    ...
    // Load SO from the new location
    int flags = RTLD_NOW;
    so = dlopen(new_path, flags);
    if (so == NULL) {
        el_critical_fmt("LIN: Could not load SO %s.\n", path);
        rc = -1;
        goto _err;
    }

    ee_init_fn *ee_init_ptr = (ee_init_fn *)dlsym(so, "ee_init");
    if (ee_init_ptr == NULL) {
        el_critical("LIN: Could not find ee_init.\n");
        rc = -1;
        goto _err;
    }
    ...
    ee_frame_fn *ee_frame_ptr = (ee_frame_fn *)dlsym(so, "ee_frame");
    if (ee_frame_ptr == NULL) {
        el_critical("LIN: Could not find ee_frame.\n");
        rc = -1;
        goto _err;
    }
...
bool ep_shared_reload(struct ep_shared_game *out_shared_game,
    ...
    // Try load the new SO, if can't, bail out and continue as nothing happened
    int rc = ep_shared_load(out_shared_game, out_shared_game->path, arena);
    if (rc != 0) {
        return false;
    }
